...
run_fs_info() {
    ...
    run_tool ./fs_make.x test2.fs 8192
    ...
    run_test ./test_fs.x info test2.fs
    ...
    local corr_array=()
    corr_array+=("FS Info:")
    corr_array+=("total_blk_count=8198")
    corr_array+=("fat_blk_count=4")
    corr_array+=("rdir_blk=5")
    corr_array+=("data_blk=6")
    corr_array+=("data_blk_count=8192")
    corr_array+=("fat_free_ratio=8191/8192")
    corr_array+=("rdir_free_ratio=128/128")
    ...
    run_test ./test_fs.x info test.fs
    echo "$(select_line "${STDERR}" "1")"
    echo "$(select_line "${STDERR}" "2")"

    run_test ./mytest_fs.x unmount test.fs
    echo "$(select_line "${STDERR}" "1")"
    echo "$(select_line "${STDERR}" "2")"
...
run_fs_info_full() {
	...
	run_tool timeout 2 ./test_fs.x add test.fs test-file-3
    run_tool timeout 2 ./test_fs.x add test2.fs test-file-1
    run_tool timeout 2 ./test_fs.x add test2.fs test-file-2
    run_tool timeout 2 ./test_fs.x add test2.fs test-file-3

	run_test ./test_fs.x info test.fs
	rm -f test-file-1 test-file-2 test-file-3
    ...
    run_tool timeout 2 ./test_fs.x rm test.fs test-file-1
    run_tool timeout 2 ./test_fs.x rm test.fs test-file-2
    run_tool timeout 2 ./test_fs.x rm test.fs test-file-3
    ...
    line_array+=("$(select_line "${STDOUT}" "8")")
    local corr_array=()
    corr_array+=("fat_free_ratio=8187/8192")
    corr_array+=("rdir_free_ratio=125/128")

    sub=0
    compare_output_lines line_array[@] corr_array[@] "0.5"

    rm -f test.fs test2.fs
